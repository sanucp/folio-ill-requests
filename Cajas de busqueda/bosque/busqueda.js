...
function searchCatalog() {
    ...
    var url = '';
    if (rev1[0].checked == true) { url = "https://bosq.ent.sirsi.net/client/es_ES/default/search/results?" + "qu=" + text1 + "&te=" ; }
    if (rev1[1].checked == true) { url = "https://bosq.ent.sirsi.net/client/es_ES/default/search/results?" + "qu=" + text1 + "&te=&rt=false%7C%7C%7CTITLE%7C%7C%7CTítulo" ; }
    if (rev1[2].checked == true) { url = "https://bosq.ent.sirsi.net/client/es_ES/default/search/results?" + "qu=" + text1 + "&rt=false%7C%7C%7CAUTHOR%7C%7C%7CAutor" ; }
    window.open (url);}
}
function googleSearch(form, buff){
	var gtext = chkObject(form.google1, '');
	gtext = encodeURI(gtext);
    //var gtext = document.getElementById("google1").value;
    if (gtext != ''){
		var gurl = "https://scholar.google.com/scholar?hl=en&as_sdt=0%2C5&" + "q=" + gtext + "&btnG=";
		window.open (gurl);
	}
	else {
