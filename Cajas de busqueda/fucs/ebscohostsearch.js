...
function ebscoHostSearchParse(ebscohostsearchtext, ebscohostsearchmode) {
  ...
  for (var i = 0; i < sT.length; i++) {
    ...
    switch (sT[i].toUpperCase()) {
      ...
      case "TW":
      case "TY2":
        if (sQ.length > nP) if (bP == 0) sQ += ebscohostsearchmode;
      ...
      default:
        if (
          sT[i].toLowerCase().search(/^w([01]?\d\d?|2[0-4]\d|25[0-5])$/) !=
            -1 ||
          sT[i].toLowerCase().search(/^n([01]?\d\d?|2[0-4]\d|25[0-5])$/) != -1
        ) {
          sQ += "+" + sT[i] + "+";
          bP = 1;
          break;
...
function ebscoHostSearchGo(form) {
  function ebscoAddDisciplines() {
    function addScrollBoxLimiters(disciplinesDiv) {
      var visibleDisciplines = document.getElementById(
          "ehostVisibleDisciplines"
        ),
        options = visibleDisciplines
          ? visibleDisciplines.getElementsByTagName("option")
          : [],
      ...
      if (options.length) {
        for (var i = 0; i < options.length; i++) {
          ...
          if (
            (theOption.selected ||
              (theOption.getAttribute("selected") === "selected" &&
                theOption.style.display === "none")) &&
            code !== ""
          ) {
            // Do not include default option
        ...
        if (selectedOptionValues.length) {
          ...
          for (var i = 0; i < selectedOptionValues.length; i++) {
            ebscohosturl +=
              "LO+system.dis-" + selectedOptionValues[i].toLowerCase() + "%7e";
          }
        }
      ...
      function addDisciplinesToUrl() {
        ...
        var hiddenDisciplines = disciplinesDiv
          .getElementById("ehostHiddenDisciplines")
  ...
  var htmlSelector = document.getElementById("guidedField_3");
...
function getMultiple(ob) {
  ...
  for (var i = 0; i < ob.length; i++)
    if (ob[i].checked) arSelected.push(ob[i].value);
  return arSelected;
}
...
function highlight(id, currentClass, checkboxId) {
  ...
  if (checkbox_element.checked == false) findObj("cball").checked = false;
}
