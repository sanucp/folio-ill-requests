...
if ($ext_correcta  && $tamano <= $limite) {
  if ($_FILES['archivo']['error'] > 0) {
    ...
    move_uploaded_file(
      $nombre_tmp,
      'subidas/data/' . $nombre_int
    );
    // Inicio		
    $registros = array();
    if (($fichero = fopen('subidas/data/' . $nombre_int, "r")) !== FALSE) {
...
BODY;
      ...
      $titulos = "";
      $arch_salida = fopen("arreglado/data/" . $nombre_int, "w");
      for ($j = 0; $j < $num_campos; $j++) {
        if (($j !== 2) and ($j !== 3) and ($j < 19)) {
      ...
      $filePath = 'arreglado/data/' . $fileName;
